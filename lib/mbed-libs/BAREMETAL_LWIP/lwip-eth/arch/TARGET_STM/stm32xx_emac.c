...
//#include "lwip/tcpip.h"   [iot2-debug]
#include "lwip/ethip6.h"
#include <string.h>
//#include "cmsis_os2.h"
#include "mbed_interface.h"
#include "eth_arch.h"   //[iot2-debug]
//=====================================iot2-debug===================================================
...
#define MAC_ADDR5   0U
    ...
    EthHandle.Init.PhyAddress = DP83848_PHY_ADDRESS; //[iot-debug]
...
#else // defined(TARGET_STM32F469NI), [iot-debug]
...
#endif  //defined(TARGET_STM32F469NI), [iot-debug]
}
 ...
 */
void _eth_arch_ethernetif_input(struct netif *netif)
{
    err_t err;
    struct pbuf  *p;

    // Get pbuf for the new packet
    p = _eth_arch_low_level_input(netif);

    // No packet is received, return
    if (p == NULL){
        return;
    }

    // A non-empty packet is received, call the entry point of LwIP
...
/**[iot2-debug]: modify flags for phy here!!----> ethernetif_set_link
...
/**
  ...
  * @retval None
  */
void eth_arch_enetif_set_link(struct netif *netif)
{
  uint32_t regvalue = 0;
...
void eth_arch_enetif_update_config(struct netif *netif){
  ...
  { 
    ...
    if(EthHandle.Init.AutoNegotiation != ETH_AUTONEGOTIATION_DISABLE)
    {
      ...
      do
      {
        ...
        if((sys_now() - tickstart ) > 1000) //HAL_GetTick()
        {
          /* In case of timeout */
          goto error;
        }
