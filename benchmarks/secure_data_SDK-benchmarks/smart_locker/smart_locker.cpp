...
void benchmarkMain(void){
    ...
    iot2CallSVC(IOT2_RESERVED_SVC,DISABLE_SECURE_DATA_SVC_NUM);// disable secure region// disable secure region
    //#sdio

    // clear up buffers
    memset(tcp_buff, 0, 128);
    memset(rtc_buff, 0, sizeof(rtc_buff));

    // initialize Ethernet/TCP
    iot2InitTCP(&eth, &socket);
    iot2recv(&socket, ack, sizeof(ack));
    ...
    for (uint8_t i = 0; i < DATASET_SIZE; i++){
        ...
        int_bytes = snprintf(tcp_buff, 128, 
        ...
        int_bytes = iot2send(&socket, tcp_buff, 128);
        if( int_bytes < 0){
            iot2SerialDebugMsg("[SEND ERROR]: int_bytes < 0");
            iot2Error();
        }

        // temp fix to lwip bug
        iot2recv(&socket, ack, sizeof(ack));
    ...
    for (uint8_t i = 0; i < NUM_SERVER_INQUIRIES; i++){
        int_bytes = iot2recv(&socket, tcp_buff, 128);
        ...
        // handle sent cmd
        handleSmartLockerReq(tcp_buff, 128);
        // send the response
        int_bytes = iot2send(&socket, tcp_buff, 128);
        if( int_bytes < 0){
            iot2Error();
    ...
    snprintf(tcp_buff, 128, IoT2_TCP_DRIVER_END_MSG);
    // send the information to the server
    int_bytes = iot2send(&socket, tcp_buff, 128);
    if( int_bytes < 0){
        iot2SerialDebugMsg("[SEND ERROR]: int_bytes < 0");
