...
#if (defined(IOT2_IO_METRICS))
    // Timestamp for throughput measurement
    IoT2GlobalCounter.IoT2ThroughputCycles.startCntr = DWT->CYCCNT;
#endif
    _lock.lock();
...
#if (defined(IOT2_IO_METRICS))
    // end measuring cycles for throughput
    IoT2GlobalCounter.IoT2ThroughputCycles.metricCntr = DWT->CYCCNT -
                                                IoT2GlobalCounter.IoT2ThroughputCycles.startCntr;
    // if return is > 0 (no error) add it to throughput bytes
    IoT2GlobalCounter.IoT2throughputBytes += (( ret > 0 ) ? ret : 0);
#endif
    return ret;
}
